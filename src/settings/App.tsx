...
export class App extends Component<{}, AppState> implements OC.Plugin<OC.Search.Core> {
	...
	createRow = (event: FormEvent) => {
		...
		this.api.createFolder(mountPoint).then((id) => {
			...
			folders.push({
				...
				groups: {admin:OC.PERMISSION_ALL},
                users: {},
                quota: -3,
                size: 0,
                id,
            ...
            this.api.setPermissions(id, "admin", OC.PERMISSION_ALL);
        });
    };
	...
	deleteFolder(folder: Folder) {
		OC.dialogs.confirm(
			t('grouprepos', 'Are you sure you want to delete "{folderName}" and all files inside? This operation cannot be undone', {folderName: folder.mount_point}),
			t('grouprepos', 'Delete "{folderName}"?', {folderName: folder.mount_point}),
			confirmed => {
				if (confirmed) {
                ...
                return <tr key={id}>
                    ...
                    <td className="remove">
                        <a className="icon icon-delete icon-visible"
                            ...
                            title={t('grouprepos', 'Delete')} />
                    </td>
                </tr>
            });

        return <div id="grouprepos-react-root"
            onClick={() => {
                        ...
                        <td>
                            <form action="#" onSubmit={this.createRow}>
                                <input
                                    ...
                                    title={t('grouprepos', 'Folder name')}
                                    placeholder={t('grouprepos', 'Folder name')}
                                    onChange={(event) => {
                                        this.setState({ newMountPoint: event.target.value })
                                    }} />
                                <input type="submit"
                                    value={t('grouprepos', 'Create')} />
                            </form>
                        </td>
                        <td colSpan={3} />
                    </tr>
