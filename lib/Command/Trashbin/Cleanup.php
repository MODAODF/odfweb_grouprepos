...
namespace OCA\GroupRepos\Command\Trashbin;
...
class Cleanup extends Base {
	...
	protected function configure() {
		$this
			->setName('grouprepos:trashbin:cleanup')
			->setDescription('Empty the grouprepo trashbin')
			->addArgument('folder_id', InputArgument::OPTIONAL, 'Id of the grouprepo')
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		if (!$this->trashBackend) {
			$output->writeln('<error>files_trashbin is disabled: group repos trashbin is not available</error>');
			return -1;
		}
		...
		if ($input->getArgument('folder_id') !== null) {
			...
			foreach ($folders as $folder) {
				if ($folder['id'] === $folderId) {
					$question = new ConfirmationQuestion('Are you sure you want to empty the trashbin of your group repo with id ' . $folderId . ', this can not be undone (y/N).', false);
			...
			$question = new ConfirmationQuestion('Are you sure you want to empty the trashbin of your group repos, this can not be undone (y/N).', false);
			if (!$input->getOption('force') && !$helper->ask($input, $output, $question)) {
				return -1;
			}
