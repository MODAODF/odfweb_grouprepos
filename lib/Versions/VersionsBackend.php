...
namespace OCA\GroupRepos\Versions;
...
class VersionsBackend implements IVersionBackend {
			...
			try {
				...
				return array_map(function (Node $versionFile) use ($file, $user, $folderId) {
					if ($versionFile instanceof Folder) {
						$this->logger->error('Found an unexpected subfolder inside the grouprepo version folder.');
					}
	...
	public function getVersionFile(IUser $user, FileInfo $sourceFile, $revision): File {
		...
		if (!($mount instanceof GroupMountPoint)) {
			throw new \LogicException('Trying to getVersionFile from a file not in a mounted group repo');
		}
		try {
			/** @var Folder $versionsFolder */
